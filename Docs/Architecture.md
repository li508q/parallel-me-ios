...
`MeetingPaperContextPresentationSnapshot` derives the active-paper context card, including summary copy, step count, close action, export action, runtime provider/context rows, and timeline disclosure title, so SwiftUI does not duplicate paper readability or export-entry rules.
...
`RoundtableActionAvailabilitySnapshot` derives whether roundtable moves and inquiry entry are safe, so restored states with partial openings or missing task context cannot expose misleading move buttons. `RoundtableControlsPresentationSnapshot` derives the visible roundtable action labels, icons, prompts, and input-dependent send gates from that availability snapshot. `RoundtableStagePresentationSnapshot` derives the roundtable shell title, status block, status tone, and busy-aware control-panel lock from the same Core rules.
...
`MeetingLibrarySnapshot` groups, status-filters, and full-text searches saved papers into recent, unfinished, and archived sections. `MeetingLibraryPresentationSnapshot` derives home-library status copy, search and filter chrome, empty states, and visible groups from the filtered and source libraries. `MeetingSummary` derives a searchable text index from the full meeting state, keeping home library behavior out of SwiftUI.
...
`RuntimePreferencesActionAvailabilitySnapshot` blocks invalid OpenAI-compatible settings from being saved and locks provider/context editing while a runtime preference operation is in flight. `RuntimePreferencesPresentationSnapshot` derives the provider-settings panel labels, OpenAI field visibility, save/clear actions, and status-message chrome from that availability rule.
...
`SettlementStageSnapshot` keeps restored settlement-stage papers from rendering a blank body when the Heart Settlement payload is missing. `HeartSettlementSnapshot` derives the fixed five-module display rows from a settlement, so the settlement editor, archived detail, and Markdown export share one canonical module order and title set. `SettlementRevisionDraft` owns settlement editing state and validation, `SettlementActionAvailabilitySnapshot` derives busy-aware apply/archive availability, and `SettlementControlsPresentationSnapshot` derives the visible action labels and icons so SwiftUI can render editable modules without deciding which actions are safe.
...
- Paper library grouping, ordering, status filtering, search filtering, and search/filter presentation chrome are derived in Core and tested without UI.
