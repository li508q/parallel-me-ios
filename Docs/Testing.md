...
- Home start presentation derives header copy, starter prompt visibility/accessibility, petition editability, and start action controls from Core.
- Issue definition presentation derives stage-one visible modes, retry recovery, loading copy, proposal revision controls, and confirmation action chrome in Core.
...
- `IssueProposalSnapshot` preserves the four-key issue proposal display order shared by the defining UI, archive detail, and Markdown export.
...
- Roundtable action availability, stage presentation, and controls presentation explain incomplete openings, missing task frames, busy state, status tone, action labels, icons, prompts, and input-dependent send gates before submitting roundtable moves or entering inquiry.
- Final inquiry has no global maximum question count.
- Voice role contracts preserve complete fixed-voice coverage, distinct role boundaries, prompt-ready text, and shared use by provider prompts.
- Provider prompt specs preserve fixed voices, shared role contracts, free-text exits, no hard inquiry cap, context boundaries, non-template inquiry rules, and required settlement modules.
...
- Early definition provider responses cannot mark a proposal ready before the local stage-one evidence guard passes; the session harness retries the provider with `harnessFeedback` instead of generating local recovery questions.
- Contradictory inquiry provider responses cannot mark settlement ready while also introducing unanswered follow-up questions, and duplicate-only inquiry attempts are retried through the same harness.
- OpenAI-compatible provider tests assert request URL, headers, body shape, strict JSON response format, balanced fenced JSON extraction, one-shot JSON repair, and HTTP error body propagation without live network calls.
...
- Runtime preference action availability and presentation block invalid OpenAI-compatible settings, derive provider/context field labels, and lock provider/context editing while async preference work is in flight.
...
- Paper context presentation derives active-paper summary copy, runtime provider/context rows, export controls, close action availability, and recent/full timeline controls in Core.
...
- Roundtable transcript grouping covers openings, fixed opening detail rows, user moves, model replies, and legacy ungrouped turns.
- Meeting archive snapshot and presentation derive archived detail rows, section order, full timeline title, reset action, and user settlement revisions from Core state.
- Resume meeting presentation derives continuation actions, deletion actions, and delete-confirmation copy from Core.
...
- `HeartSettlementSnapshot` preserves the fixed five-module display order shared by the settlement editor, archive detail, and Markdown export.
...
- Settlement action availability and controls presentation lock apply/archive actions while async work is in flight and keep action labels/icons in Core.
- Meeting library grouping, sorting, summary search, full-paper content search, and home-library presentation/search/filter copy are tested in Core.
...
Current smoke coverage includes 103 checks across flow rules, app privacy manifest metadata, fixed voice role contracts, starter prompts, start readiness and blocked start attempts, home start presentation, busy input locking, issue definition presentation, definition retry recovery, definition response guarding, stage-one evidence guarding, issue proposal display snapshots, definition harness retry, inquiry retry recovery, inquiry harness retry, in-flight activity snapshots, stage-one answer batching, inquiry answer batching, inquiry presentation, answer-batch presentation, proposal confirmation availability, stage progress, paper context presentation, roundtable inquiry readiness, roundtable action availability, roundtable controls presentation, roundtable stage presentation, settlement request availability, inquiry response guarding, runtime snapshots, restored-paper runtime rebuilding, failed runtime rebuild persistence, missing restored-paper feedback, lifecycle timestamps, paper library grouping, status filtering, home-library presentation copy, paper-library search/filter chrome, resume-card presentation, full-paper search, and paper-library action availability, archived detail snapshots, archive presentation, export availability, Markdown export, export file writing, file repository resilience, provider prompt contracts, provider configuration, provider settings normalization, OpenAI-compatible transport, OpenAI-compatible JSON extraction and repair, secure settings persistence, provider context persistence, runtime preference actions, runtime preference presentation, runtime preference action availability, runtime preference save gating, runtime load error copy, deduplication, unanswered-question coverage, free-text answers, proposal refinement, resume policy, meeting timeline, roundtable transcript grouping, voice opening display snapshots, settlement readiness, non-substantive action answers, settlement stage recovery, heart settlement display snapshots, settlement revision drafts, settlement action availability, settlement controls presentation, archive completeness, user settlement revisions, archive summaries, repositories, session events, session diagnostics, meeting state health, and all demo roundtable move types.
